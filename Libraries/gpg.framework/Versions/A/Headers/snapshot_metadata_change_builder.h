...
class GPG_EXPORT SnapshotMetadataChange::Builder {
 public:
  ...
  /**
   * A concise description of the snapshot metadata_change.
   */
  Builder &SetDescription(std::string const &description);

  /**
   * The duration of the snapshot metadata_change.
   */
  Builder &SetDuration(gpg::Duration duration);

  /**
   * The raw bytes of the encoded png cover image of the snapshot metadata
   * change. Cover image must be less than 800 kb. If it is not it
   * will be scaled. Image must be set every commit or it will be cleared.
   */
