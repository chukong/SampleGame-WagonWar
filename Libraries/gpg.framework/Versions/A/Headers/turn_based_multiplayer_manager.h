...
/**
   * Asynchronously takes the local participant's turn. When taking a turn, the
   * participant may specify new values for  <code>match_data</code> and for
   * <code>ParticipantResults</code>. When the turn is over, the updated match
   * is returned via the <code>TurnBasedMatchCallback</code>. This function can
   * only be called when <code>TurnBasedMatch::Status()</code> is
   * <code>MatchStatus::MY_TURN</code>, and
   * <code>TurnBasedMatch::AutomatchingSlotsAvailable()</code> is greater than
   * 0.
   *
   * @param match The match where the turn takes place.
   * @param match_data A blob of data to send to the next participant.
   ...
   *     <code>TurnBasedMatchResponse</code>.
   */
  void TakeMyTurnAndAutomatch(TurnBasedMatch const &match,
                              std::vector<uint8_t> match_data,
                              ParticipantResults const &results,
                              TurnBasedMatchCallback callback);

  /**
   * Blocking version of {@link TakeMyTurnAndAutomatch}. Allows the caller to
   * specify a timeout in ms. After the specified time elapses, the function
   * returns <code>ERROR_TIMEOUT</code>.
   */
  TurnBasedMatchResponse TakeMyTurnAndAutomatchBlocking(
      Timeout timeout,
      TurnBasedMatch const &match,
      std::vector<uint8_t> match_data,
      ParticipantResults const &results);
  ...
  /**
   * Overload of {@link TakeMyTurnAndAutomatchBlocking}, which uses a default
   * timeout of 10 years.
   */
  TurnBasedMatchResponse TakeMyTurnAndAutomatchBlocking(
      TurnBasedMatch const &match,
 ...
 /**
  ...
  /**
   * Asynchronously leaves a match during the local participant's turn, and
   * selects the next player via auto-matching. The response returned via the
   * <code>TurnBasedMatchCallback</code> contains the state of the match after
   * the local participant has left. If this departure leaves the match with
   * fewer than two participants, the match is canceled. This function can only
   * be called when <code>TurnBasedMatch::Status()</code> returns
   * <code>MatchStatus::MY_TURN</code>, and
   * <code>TurnBasedMatch::AutomatchingSlotsAvailable()</code> returns a value
   * greater than 0.
   *
   * @param match The match to leave.
   * @param callback The callback that receives the
   *     <code>TurnBasedMatchResponse</code>.
   */
  void LeaveMatchDuringMyTurnAndAutomatch(TurnBasedMatch const &match,
                                          MultiplayerStatusCallback callback);

  /**
   * Blocking version of {@link LeaveMatchDuringMyTurnAndAutomatch}. Allows the
   * caller to specify a timeout in ms. After the specified time elapses, the
   * function returns <code>ERROR_TIMEOUT</code>.
   */
  MultiplayerStatus LeaveMatchDuringMyTurnAndAutomatchBlocking(
      Timeout timeout, TurnBasedMatch const &match);

  /**
   * Overload of {@link LeaveMatchDuringMyTurnAndAutomatchBlocking}, which uses
   * a default timeout of 10 years.
   */
  MultiplayerStatus LeaveMatchDuringMyTurnAndAutomatchBlocking(
      TurnBasedMatch const &match);
  ...
  struct TurnBasedMatchesResponse {
    ...
    /**
     * A vector of <code>MultiplayerInvitation objects</code>. If
     * <code>IsSuccess(status)</code> returns true, this vector contains
     * invitations. Otherwise, it is empty.
     */
    std::vector<MultiplayerInvitation> invitations;

    /**
     * A vector of <code>TurnBasedMatch</code> objects. If
     * <code>IsSuccess(status)</code> returns true, this vector contains
     * matches. Otherwise, it is empty.
     */
    std::vector<TurnBasedMatch> matches;
  };
  ...
  // TODO(ericrk): Add filtering by match type to FetchMatches b/13785460.
  /**
  ...
  struct MatchInboxUIResponse {
     ...
     * <code>Valid()</code> only returns true for the invitation if
     * <code>IsSuccess()</code> returns true, and <code>match.Valid()</code>
     * does not.
     */
     ...
     * The <code>TurnBasedMatch</code> for this response. <code>Valid()</code>
     * only returns true for the match if <code>IsSuccess()</code> returns true,
     * and <code>invitation.Valid()</code> does not.
     */
    TurnBasedMatch match;
  };
