...
class GPG_EXPORT TurnBasedMatch {
 public:
  ...
  /**
   * Similar to {@link Data}, but moves the data out of the
   * <code>TurnBasedMatch</code> object, returning it to the caller. This can be
   * used to avoid copies when updating match data. {@link Valid} must return
   * true for this function to be usable.
   */
