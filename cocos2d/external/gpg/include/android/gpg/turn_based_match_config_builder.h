...
class GPG_EXPORT TurnBasedMatchConfig::Builder {
 public:
  ...
  /**
   * A bit mask indicating exclusive roles for players. (E.g. For example, if one
   * player is the attacker, the other must be the defender.) Successful
   ...
   * default, no players are added. (TODO ericrk: same question as above)
   */
  Builder &AddAllPlayersToInvite(std::vector<std::string> const &player_ids);

  /**
   * Populates (TODO: ericrk these are values that the user inputs via UI? What are
   * being populated into? A TBMP object?) values from a
   * TurnBasedMultiplayerManager::ShowPlayerSelectUIResponse. Note that this
   * does not populate the variant or the exclusive bit mask.
   */
