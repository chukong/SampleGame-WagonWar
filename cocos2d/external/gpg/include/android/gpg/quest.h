...
class GPG_EXPORT Quest {
 public:
  ...
  /**
   * Returns the quest state: upcoming, open, accepted, completed, expired, or
   * failed.
   * <code>Quest::Valid()</code> must return true for this function to be
  ...
  /**
   * Returns the time (in milliseconds since epoch) at which the user is to be
   * notified that the quest is expiring soon.
   * <code>Quest::Valid()</code> must return true for this function to be
   * usable.
   */
  Timestamp ExpirationNotificationTime() const;
